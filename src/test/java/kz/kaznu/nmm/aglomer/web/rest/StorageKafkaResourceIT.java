...
class StorageKafkaResourceIT {
    ...
    void setup() {
        ...
        StorageKafkaResource kafkaResource = new StorageKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/storage-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/storage-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
